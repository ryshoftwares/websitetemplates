/* ==========================================================================
   Sam Rivera — Minimal Professional personal portfolio
...
(function () {
  ...
  if (skillRows.length && "IntersectionObserver" in window) {
    var skillObserver = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) { entry.target.classList.add("is-visible"); skillObserver.unobserve(entry.target); }
      });
    }, { threshold: 0.4 });
    skillRows.forEach(function (row) { skillObserver.observe(row); });
  } else {
    skillRows.forEach(function (row) { row.classList.add("is-visible"); });
  }
