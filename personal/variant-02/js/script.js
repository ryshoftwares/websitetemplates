/* ==========================================================================
   JOON. — Creative Bold personal brand
...
(function () {
  ...
  /* ---- proficiency bars ---- */
  var profRows = document.querySelectorAll(".proficiency__row");
  if (profRows.length && "IntersectionObserver" in window) {
    var profObserver = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) { entry.target.classList.add("is-visible"); profObserver.unobserve(entry.target); }
      });
    }, { threshold: 0.4 });
    profRows.forEach(function (row) { profObserver.observe(row); });
  } else {
    profRows.forEach(function (row) { row.classList.add("is-visible"); });
  }
